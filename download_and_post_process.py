...
doc_links = dict()
docs_texts = {"Aktuell gültige Dokumente": "current",
              "Zukünftig gültige Dokumente": "future",
              "Archivierte Dokumente": "past"}
for doc_text in docs_texts:
    doc_links[docs_texts[doc_text]] = docs_overview_soup.find("a", string=re.compile(r'\s*' + doc_text + r'\s*')).attrs[
        'href']
for doc_link in doc_links:
    ...
    download_table = docs_soup.find("table", {"class": "table table-responsive table-condensed"})
    for table_line in download_table.find_all("tr"):
        ...
        try:
            publ_date = datetime.datetime.strptime(table_cells[1].text.strip(), "%d.%m.%Y")
        except ValueError:
            continue
        try:
            vali_date = datetime.datetime.strptime(table_cells[2].text.strip(), "%d.%m.%Y")
        ...
        file_name = doc_name + "_" + vali_date.strftime("%Y%d%m") + "_" + publ_date.strftime("%Y%d%m") + ".pdf"
